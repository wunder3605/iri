...
do
    ...
    do
        ...
        then
            ...
            sudo cp ./nginx/nginx3.conf /etc/nginx/nginx.conf 
        ...
        then
            ...
            sudo cp ./nginx/nginx4.conf /etc/nginx/nginx.conf 
        elif [ ${TOPOLOGY} != "4_clique" -a ${TOPOLOGY} != "4_circle" -a ${TOPOLOGY} != "3_clique" ];
        then
            echo "configure 7" ${TOPOLOGY} ${DATA}
            sudo cp ./nginx/nginx7.conf /etc/nginx/nginx.conf 
        fi
        sudo nginx -s stop 
        sleep 2 
        sudo nginx
        ...
        # configure topology
        sudo cp ./topology/${TOPOLOGY} topology.txt 
