...
def deploy_iri_server():
    ...
    for ip_address in ip_pub:
        ...
        if int(num_exist):
            ...
            cmd("-i", "-H", "trust@" + ip_address, "-x", "-oStrictHostKeyChecking=no", "sudo docker run  -d -p 14700:14700 -p 13700:13700 --name iota-node -v /home/trust/iri/scripts/examples/data:/iri/data -v /home/trust/iri/scripts/examples/conf/neighbors iota-node:v0.1-streamnet  /entrypoint.sh")
        else:
            exitflag = Popen(["pssh", "-i", "-H", "trust@" + ip_address, "-x", "\"-oStrictHostKeyChecking=no\"","sudo docker ps -a |grep iota-node |wc -l "], shell=False, stdout=PIPE, stderr=STDOUT)
            exitflag = exitflag.stdout.readlines()[1].strip()
            if int(exitflag):
                cmd("-i", "-H", "trust@" + ip_address, "-x", "-oStrictHostKeyChecking=no", "sudo docker rm iota-node")
            cmd("-i", "-H", "trust@" + ip_address, "-x", "-oStrictHostKeyChecking=no","sudo docker run  -d -p 14700:14700 -p 13700:13700 --name iota-node -v /home/trust/iri/scripts/examples/data:/iri/data -v /home/trust/iri/scripts/examples/conf/neighbors iota-node:v0.1-streamnet  /entrypoint.sh")
    return 'success'
...
def clear_iri_server():
    ...
    for ip_address in ip_pub:
        ...
        num_exist = oret.stdout.readlines()[1].strip()
        if int(num_exist):
            cmd("-i", "-H", "trust@" + ip_address, "-x", "-oStrictHostKeyChecking=no", "sudo rm -rf /home/trust/iri/scripts/examples/data/testnetdb")
            cmd("-i", "-H", "trust@" + ip_address, "-x", "-oStrictHostKeyChecking=no","sudo rm -rf /home/trust/iri/scripts/examples/data/testnetdb.log")
            cmd("-i", "-H", "trust@"+ip_address, "-x", "-oStrictHostKeyChecking=no","sudo docker stop iota-node")
            cmd("-i", "-H", "trust@" + ip_address, "-x", "-oStrictHostKeyChecking=no", "sudo docker rm iota-node")
        else:
            cmd("-i", "-H", "trust@" + ip_address, "-x", "-oStrictHostKeyChecking=no", "sudo rm -rf /home/trust/iri/scripts/examples/data/testnetdb")
...
def deploy_cli_server():
    ...
    ip_pub = list(ip_total.values())
    for ip_address in ip_pub:
        oret = Popen(["/usr/local/bin/pssh", "-i", "-H", "trust@" + ip_address, "-x", "\"-oStrictHostKeyChecking=no\"","sudo docker ps |grep iota-cli |wc -l "], shell=False, stdout=PIPE,stderr=STDOUT)
        num_exist = oret.stdout.readlines()[1].strip()
        if int(num_exist):
            ...
            exitflag = Popen(["pssh", "-i", "-H", "trust@" + ip_address, "-x", "\"-oStrictHostKeyChecking=no\"", "sudo docker ps -a |grep iota-cli |wc -l "], shell=False, stdout=PIPE, stderr=STDOUT)
            exitflag = exitflag.stdout.readlines()[1].strip()
            if int(exitflag):
                cmd("-i", "-H", "trust@" + ip_address, "-x", "-oStrictHostKeyChecking=no", "sudo docker rm iota-cli")
            cmd("-i", "-H", "trust@" + ip_address, "-x", "-oStrictHostKeyChecking=no","sudo docker run -d -p 5000:5000 --name iota-cli iota-cli:v0.1-streamnet /docker-entrypoint.sh")
