        ...
        try {
            ...
            while (!shutdown && !neighbor.isStopped()) {
                ...
                if (node.optimizeNetworkEnabled) {
                    while (((count = stream.read(data, offset, (node.transactionSize - offset + ReplicatorSinkProcessor.CRC32_BYTES))) != -1)
                            && (offset < (packetSize + ReplicatorSinkProcessor.CRC32_BYTES))) {
                        offset += count;

                        log.info("count = {}, offset = {}", count, offset);
                        // FIXME: may got wrong message according to size
                        if (offset == node.transactionSize + ReplicatorSinkProcessor.CRC32_BYTES
                ...
                int dataLen;
                if (node.optimizeNetworkEnabled) {
                    dataLen = offset - ReplicatorSinkProcessor.CRC32_BYTES;
                } else {
                    dataLen = packetSize;
                }
