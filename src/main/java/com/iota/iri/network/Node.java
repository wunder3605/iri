...
public class Node {
    ...
    private boolean checkIfReady(TransactionViewModel model) {
        ...
        try {
            if(cached==tot) {
                ...
                log.info("[zhaoming] {} {} {} {}", trunk, branch, hasTrunk, hasBranch);
                return (hasTrunk && hasBranch);
            }
        } catch(Exception e) {
            log.error("Some thing goes wrong here {}", tot-1);
            e.printStackTrace();
        }
        return false;
    }
    ...
    private synchronized void checkPersist() {
        log.info("[zhaoming] has " + bundleCache.size());
        for(Hash h : bundleCache.keySet()) {
            for(Integer i : bundleCache.get(h).keySet()) {
                ...
                if(checkIfReady(model)) {
                    ...
                    hashesToRequest.addAll(getMissingHash(model));
                }
                ...
                try {
                    ...
                    if(!hashes.isEmpty()) {
                        for(Hash h : hashes) {
                            ...
                            log.info("[zhaoming] com here to get last bundle! {}", h);
    ...
    private Runnable spawnTipRequesterThread() {
        return () -> {
            ...
            while (!shuttingDown.get()) {
                ...
                try {
                    Hash hash = null;
                    if(!hashesToRequest.isEmpty()) {
                        hash = hashesToRequest.iterator().next();
                        hashesToRequest.remove(hash);
                    } 
                    if(hash == null) {
                        continue;
                    }
                    log.info("[request] " + hash);
                    ...
                    System.arraycopy(hash.bytes(), 0, tipRequestingPacket.getData(), TransactionViewModel.SIZE,
                           reqHashSize);
                    //Hash.SIZE_IN_BYTES);
    ...
    public void broadcast(final TransactionViewModel transactionViewModel, Neighbor neighbor) {
        ...
        if(n == 10 && transactionViewModel.getLastIndex() > 0) {
            return;
        }
